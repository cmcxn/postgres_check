...
1. **定时任务**：应用启动后，根据配置的 Cron 表达式定时执行检查任务（默认每 5 分钟）
2. **数据检查**：查询 `sync_operation_log` 表，统计最近 N 分钟内 `changed_at` 字段的记录数
3. **告警判断**：如果记录数为 0，说明数据未更新
4. **邮件通知**：发送告警邮件到配置的收件人邮箱
