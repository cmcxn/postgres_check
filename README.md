...
5. **告警发送**：首次检测到无数据时发送告警邮件，系统进入告警状态
6. **告警状态**：在告警状态期间，不会重复发送告警邮件
7. **恢复检测**：当数据恢复时，计算故障持续时间
8. **恢复通知**：发送恢复邮件，包含故障时长统计
9. **每日健康检查**：每天定时（默认8:00）发送系统正常运行的邮件
10. **关闭通知**：应用关闭时发送通知邮件

详细说明请参考：[NEW_FEATURES.md](NEW_FEATURES.md)
